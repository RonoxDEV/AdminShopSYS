...
public class Item5EDITScreen extends AbstractContainerScreen<Item5EDITMenu> implements AdmModScreens.ScreenAccessor {
	...
	public Item5EDITScreen(Item5EDITMenu container, Inventory inventory, Component text) {
	...
	public void init() {
		super.init();
		Money = new EditBox(this.font, this.leftPos + 42, this.topPos + 54, 118, 18, Component.translatable("gui.adm.item_5_edit.Money"));
		Money.setMaxLength(8192);
		Money.setResponder(content -> {
			if (!menuStateUpdateActive)
				menu.sendMenuStateUpdate(entity, 0, "Money", content, false);
		});
		Money.setHint(Component.translatable("gui.adm.item_5_edit.Money"));
		this.addWidget(this.Money);
		button_done = Button.builder(Component.translatable("gui.adm.item_5_edit.button_done"), e -> {
			if (true) {
				PacketDistributor.sendToServer(new Item5EDITButtonMessage(0, x, y, z));
				Item5EDITButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
