...
public class RegisterItem12EDITProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		AdmModVariables.MapVariables.get(world).Item12 = (entity instanceof Player _plrSlotItem && _plrSlotItem.containerMenu instanceof AdmModMenus.MenuAccessor _menu ? _menu.getSlots().get(0).getItem() : ItemStack.EMPTY).copy();
		AdmModVariables.MapVariables.get(world).syncData(world);
		AdmModVariables.MapVariables.get(world).Item12NSTACK = getAmountInGUISlot(entity, 0);
		AdmModVariables.MapVariables.get(world).syncData(world);
		AdmModVariables.MapVariables.get(world).Item12MONEY = (entity instanceof Player _entity && _entity.containerMenu instanceof AdmModMenus.MenuAccessor _menu) ? _menu.getMenuState(0, "Money", "") : "";
		AdmModVariables.MapVariables.get(world).syncData(world);
